...
# 플레이어 총알 방향
bullet_direction = [0, -1]  # 기본적으로 위쪽을 향한 방향으로 설정
...
def spawn_enemy():
    x_pos = random.randint(-enemy_size, WIDTH)  # 화면 바깥쪽 좌우에서 적의 x 위치 생성
    y_pos = random.choice([-enemy_size, HEIGHT])  # 화면 바깥쪽 상단 또는 하단에서 적의 y 위치 생성
    enemy_list.append({"pos": [x_pos, y_pos], "health": enemy_health})  # 적에 체력 추가

# 총알 생성 함수 (총알의 방향을 추가)
def spawn_bullet(player_pos, direction):
    bullet_pos = [player_pos[0], player_pos[1]]
    bullets.append({"pos": bullet_pos, "direction": direction})  # 방향을 사용하여 총알 발사
...
while running:
    ...
    if not game_over:  # 게임 오버가 아닐 때만 키 입력 처리
        ...
        # 방향키로 총알 방향 설정 및 총알 발사
        if keys[pygame.K_UP]:
            bullet_direction = [0, -1]  # 위쪽
            spawn_bullet(player_pos, bullet_direction)  # 위로 총알 발사
        if keys[pygame.K_DOWN]:
            bullet_direction = [0, 1]  # 아래쪽
            spawn_bullet(player_pos, bullet_direction)  # 아래로 총알 발사
        if keys[pygame.K_LEFT]:
            bullet_direction = [-1, 0]  # 왼쪽
            spawn_bullet(player_pos, bullet_direction)  # 왼쪽으로 총알 발사
        if keys[pygame.K_RIGHT]:
            bullet_direction = [1, 0]  # 오른쪽
            spawn_bullet(player_pos, bullet_direction)  # 오른쪽으로 총알 발사

        # 총알 이동 및 충돌 처리
        for bullet in bullets[:]:
            bullet["pos"][0] += bullet["direction"][0] * bullet_speed
            bullet["pos"][1] += bullet["direction"][1] * bullet_speed
            if bullet["pos"][1] < 0 or bullet["pos"][1] > HEIGHT or bullet["pos"][0] < 0 or bullet["pos"][0] > WIDTH:
                bullets.remove(bullet)
                continue

            for enemy in enemy_list[:]:
                if detect_collision(bullet["pos"], enemy["pos"]):
                    enemy["health"] -= 1
                    bullets.remove(bullet)
                    if enemy["health"] <= 0:
    ...
    if game_over:  # 게임 오버 상태일 때
        ...
        # 총알 그리기
        for bullet in bullets:
            pygame.draw.circle(screen, (0, 0, 255), (int(bullet["pos"][0]), int(bullet["pos"][1])), bullet_size)
